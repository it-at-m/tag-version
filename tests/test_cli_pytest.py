...
def test_load_config(mock_load_config, runner):
    ...
    mock_config = {
        ...
        "service_links": {"service1": "https://example.com/service1"},
    }
...
@patch("tag_version.cli.load_config")
def test_cli_with_push(
    mock_load_config, mock_push_tag, mock_create_tag, mock_get_tags, runner
):
...
@patch("tag_version.cli.create_git_tag")
@patch("tag_version.cli.load_config")
def test_cli_with_custom_prefix(
...
def test_cli_create_tag_error(mock_load_config, mock_create_tag, runner):
    ...
    mock_create_tag.side_effect = RuntimeError(
        "fatal: tag 'service1-1.0.1' already exists"
    )
