...
from tag_version.constants import (
    ...
    CLI_VERSION_TYPE_PATCH, CLI_INVALID_VERSION_TYPE, CLI_SELECTED_VERSION_TYPE,
    CLI_FINDING_LATEST_VERSION, CLI_LOOKING_FOR_PREFIX, CLI_FOUND_TAGS,
    CLI_VALID_VERSIONS_FOUND, CLI_VERSION_TAG_ENTRY, CLI_HIGHEST_VERSION_FOUND,
    CLI_HIGHEST_VERSION_TAG, CLI_NO_VALID_VERSION_TAGS, CLI_VERSION_UPDATE_SUMMARY,
    CLI_SERVICE_LINE, CLI_CURRENT_VERSION, CLI_NEW_VERSION, CLI_NEW_TAG,
    CLI_SUMMARY_SEPARATOR, CLI_TAG_CREATED_LOCALLY, CLI_FAILED_TO_CREATE_TAG,
    CLI_ERROR_CREATING_TAG, CLI_PUSHING_TAG, CLI_PUSH_SUCCESS, CLI_WORKFLOW_TRIGGERED,
    CLI_SERVICE_LINK, CLI_PUSH_ERROR, CLI_PUSH_ERROR_SEPARATOR,
    CLI_TAG_LOCALLY_NOT_PUSHED, CLI_CHECK_PERMISSIONS, CLI_PUSH_LATER,
...
@click.option(
    "--no-push",
    is_flag=True,
    help="Don't push the tag to remote repository"
)
def main(
    ...
    if not service and not prefix:
        ...
        if service_input.isdigit():
            ...
            if service_input in available_services:
                ...
                print_color(CLI_INVALID_SERVICE_NAME.format(service_input), color=Fore.RED)
                sys.exit(1)
    ...
    if not version_type:
        ...
        version_input = click.prompt(
            "Enter version type number or name",
            type=str
        )
        ...
        if version_input.isdigit():
            ...
            if version_input.lower() in [v.lower() for v in DEFAULT_VERSION_TYPES]:
                ...
                version_type = next(v for v in DEFAULT_VERSION_TYPES if v.lower() == version_input.lower())
            else:
                print_color(CLI_INVALID_VERSION_TYPE.format(version_input), color=Fore.RED)
                sys.exit(1)
    
    print_color(CLI_SELECTED_VERSION_TYPE.format(version_type), color=Fore.GREEN)
    
    # Get all tags from Git
    print_color(CLI_FINDING_LATEST_VERSION.format('service ' + service if service else 'custom prefix ' + prefix))
    ...
    print_color(CLI_FOUND_TAGS.format('no' if tag_count == 0 else tag_count), color=color)
    ...
    if latest_version_obj:
        ...
        for v in sorted_versions:
            print_color(CLI_VERSION_TAG_ENTRY.format(v.version_string, v.tag), color=Fore.GREEN)
        ...
        print_color(CLI_HIGHEST_VERSION_TAG.format(latest_version_obj.tag), color=Fore.GREEN)
    ...
    print_color(CLI_CURRENT_VERSION.format(latest_version_obj.version_string), color=Fore.YELLOW)
    ...
    # Ask to push the tag
    push_confirmed = yes or (not no_push and click.confirm("Do you want to push this tag to origin?", default=True))
    ...
    if push_confirmed:
        ...
        if success:
            ...
            if service and service in service_links:
                print_color(CLI_SERVICE_LINK.format(service, service_links[service]), color=Fore.CYAN)
        else:
